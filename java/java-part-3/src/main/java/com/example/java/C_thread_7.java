...
// [WAITING, TIMED_WAITING] : 스레드의 작업이 종료되지 않았지만 실행가능하지 않은(unrunnable) 일시정지 상태,
...
// -> 지정된 일시정지시간이 다되거나(time-out), notify(), resume(), interrupt()가 호출되면 일시 정지 상태를 벗어나
...
public class C_thread_7 {
    ...
    public static void exam1() {
        ThreadEx_sleep_1 th1 = new ThreadEx_sleep_1();
        ThreadEx_sleep_2 th2 = new ThreadEx_sleep_2();
        th1.start();
        th2.start();
        ...
        // th1이 가장 늦게 종료되어야 하는데 결과에서는 제일 먼저 종료가되었다.
        // 그 이유는 sleep()이 항상 현재 실행 중인 스레드에 대해 작동하기 때문에 th1.sleep()을 하였어도
    ...
    // 진행 중인 스레드의 작업이 끝나기 전에 취소시켜야할 때가 있다. 예를 들어 큰 파일을 다운로드받을 때
    // 시간이 너무 오래 걸리면 중간에 다운로드를 포기하고 취소할 수 있어야 한다.
    // interrupt()는 스레드에게 작업을 멈추라고 요청한다. 단지 멈추라고 요청만 하는 것일 뿐 스레드를 강제로 종료시키지는 못한다.
    // interrupt()는 그저 스레드의 interrupted상태(인스턴스 변수)를 바꾸는 것일 뿐이다.
    // 그리고 interrupted()는 스레드에 대해 interrupt()가 호출되었는지 알려준다. interrupt()가 호출되지 않았다면 false를,
    // 호출되었다면 true를 반환한다.
    // - interrupted() : 스레드의 현재 interrupted 상태를 반환
    // - isInterrupted() : 스레드의 현재 interrupted 상태를 반환하고, false로 변경
    public static void exam2_1() {
        ThreadEx_interrupt_1 th1 = new ThreadEx_interrupt_1();
        th1.start();
    ...
    // 스레드가 sleep(), wait(), join()에 의해 '일시 정지 상태(WAITING)'에 있을 때, 해당 스레드에 대해
    // interrupt()를 호출하면, sleep(), wait(), join()에서 InterruptedException이 발생하고 스레드는 '실행 대기 상태'로 바뀐다.
    public static void exam2_2() {
        ThreadEx_interrupt_2 th1 = new ThreadEx_interrupt_2();
        th1.start();
        System.out.println("아무 값이나 입력하세요.");
        Scanner sc = new Scanner(System.in);
        String input = sc.nextLine();
    ...
    // 즉, 실행 상태에 있는 스레드가 자신의 남은 시간을 포기하고 다른 스레드에게 기회를 주도록 스케줄러에게 신호를 보내는 역할을 한다.
    // 그러나, yield() 메서드는 단순히 힌트일 뿐이며, 반드시 다른 스레드가 CPU 시간을 받는다는 보장은 없다.
    public static void exam3() {
        // 이 코드를 실행하면, 두 스레드(t1과 t2)가 번갈아가면서 실행되는 모습을 볼 수 있다.
        // Thread.yield() 메서드가 호출될 때마다 스레드는 스케줄러에게 양보를 요청하지만,
    ...
    public static void exam4() {
        ...
        // join을 사용하지 않았다면 main스레드가 바로 종료되었겠지만, join()으로 스레드 t1, t2가 모두 작업을 마칠 때까지
        // main스레드가 기다리도록 했다.
    }
