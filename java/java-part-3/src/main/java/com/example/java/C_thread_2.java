...
// 새로운 스레드를 생성하고 실행시킬 때마다 새로운 호출스택이 생성되고 스레드가 종료되며
// 작업에 사용된 호출스택은 소멸된다.
// 스레드가 둘 이상일 때는 호출스택의 최상위에 있는 메서드일지라도 대기상태 있을 수 있다.
// 스케줄러는 실행대기중인 스레드들의 우선순위를 고려하여 실행순서와 실행시간을 결정하고,
// 각 스레드들은 작성된 스케줄에 따라 자신의 순서가 되면 지정된 시간동안 작업을 수행한다.
