...
// 그러면 스레드가 락을 얻어 해당 객체에 대한 작업을 수행할 수 있게 된다.
...
// notifyAll()은 기다리고 있는 모든 스레드에게 통보를 하지만, 그래도 lock을 얻을 수 있는 것은 하나의 스레드일 뿐이고
// 나머지 스레드는 통보를 받긴 했지만, lock을 얻지 못하면 다시 lock을 기다리게 된다.

// wait()은 notify() 또는 notifyAll()이 호출될 때까지 기다리지만,
// 매개변수가 있는 wait()은 지정된 시간동안만 기다린다. 즉 지정된 시간이 지난 후에 자동적으로 notify()가 호출되는 것과 같다.
// waiting pool은 객체마다 존재하는 것이므로 notifyAll()이 호출된다고 해서 모든 객체의 waiting pool에 있는 스레드가 깨어지는 것이 아니다.
// notifyAll()이 호출된 객체의 waiting pool에 대기 중인 스레드만 해당된다.

// ** wait()와 notify() 메서드를 사용하려면 반드시 synchronized 블록(또는 메서드) 안에서 호출해야 한다.

class Chat {
    private boolean flag = false; // false: 질문 대기, true: 답변 대기
...
class QuestionThread extends Thread {
    ...
    public void run() {
        for (String question : questions) {
            chat.question(question); // 질문을 하고 대기
        }
    }
}
